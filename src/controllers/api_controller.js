        ...
        try {
            ...
            reply_msg(res, {
                attestation: {
                    ...
                    inserted_at: moment(attestationData[0].inserted_at).format(DATE_FORMAT)
                },
                merkleproof: {
        ...
        try {
            ...
            for (let itr = 0; itr < data.length; ++itr) {
                ...
                if (!attestation) {
                    ...
                    response['data'].push({
                        commitment: data[itr].commitment,
                        date: moment(attestation.inserted_at).format(DATE_FORMAT)
                    });
                }
            }
            ...
            if (data.length === 0) {
                reply_msg(res, {
                    attestation: {
                        ...
                        inserted_at: moment(response.inserted_at).format(DATE_FORMAT)
                    },
                    attestationInfo: {
                        txid: response.txid,
                        amount: '',
        ...
        try {
            ...
            reply_msg(res, {
                blockhash: {
                    ...
                    amount: data[0].amount / 100000000,
                    blockhash: data[0].blockhash,
                    time: moment(data[0].time * 1000).format(DATE_FORMAT)
                }
            }, startTime);
        } catch (error) {
            return reply_err(res, INTERNAL_ERROR_API, startTime);

...
module.exports = {
  ...
  latest_attestation: (req, res) => {
    ...
    // TODO: Add Feature -------------------------------------------------------
    //
    reply_msg(res, 'latest_attestation', startTime);
  },
  latest_commitment: (req, res) => {
    ...
    // TODO: Add Feature -------------------------------------------------------
    //
    reply_msg(res, 'latest_commitment', startTime);
  },
  commitment_latest_proof: (req, res) => {
    ...
    models.attestation.find().sort({id: -1}).limit(1).exec((error, data) => {
      if (error) {
        console.log("sdfsadfsdfdsaf")
        return ;
      }

      if (data[0].merkle_root === String)
        console.log("eeee")

      console.log(data[0].merkle_root);
    ...
    models.merkleProof.find({position: position}, (error, data) => {
      ...
      reply_msg(res, { proof: data[0].proof }, startTime);
    });
  },

...
const MISSING_ARG_SIGNATURE_COMMITMENT =
    'missing X-MAINSTAY-SIGNATURE-COMMITMENT'
...
module.exports = {
  ...
  commitment_send: (req, res) => {
    ...
    req.on('data', chunk => {
      ...
      apiKey = data['X-MAINSTAY-APIKEY'];
      if (apiKey === undefined)
        return reply_err(res, MISSING_ARG_APIKEY, startTime);
      playload = JSON.parse(base64decode(data['X-MAINSTAY-PLAYLOAD']));
      if (playload === undefined)
        return reply_err(res, MISSING_ARG_PLAYLOAD, startTime);
      signatureApiKey = data['X-MAINSTAY-SIGNATURE-APIKEY'];
      if (signatureApiKey === undefined)
        return reply_err(res, MISSING_ARG_SIGNATURE_APIKEY, startTime);;
      signatureCommitment = data['X-MAINSTAY-SIGNATURE-COMMITMENT'];
      if (signatureCommitment === undefined)
        return reply_err(res, MISSING_ARG_SIGNATURE_COMMITMENT, startTime);
      if (playload.commitment === undefined)
        return reply_err(res, MISSING_PLAYLOAD_COMMITMENT, startTime);
      if (playload.position === undefined)
        return reply_err(res, MISSING_PLAYLOAD_POSITION, startTime);
      models.clientDetails.find({ client_position: playload.position },
                                (error, data) => {
        if (error)
          return reply_err(res, INTERNAL_ERROR_API, startTime);
        ///
        /// TODO: add Verified API Key
        ///
        let msg = new message(playload.commitment);
        if (msg.verify(data[0].pubkey, signatureCommitment)) {
          let clientCommitment = new models.clientCommitment({
            client_position: playload.position,
            commitment: playload.commitment
          });
